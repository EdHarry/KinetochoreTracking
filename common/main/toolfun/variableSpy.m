function varargout = variableSpy(varargin);
%VARIABLESPY creates an edit window for interactive editing of variables
%      This functions allows you to edit variables in a spreadsheet like way.
%      To edit a variable 'in1', type:
%
%        out1 = variableSpy(in1);
%
...
%      parameters of variableSpy:
%
%        [out1,out2,out3,out4] = variableSpy(in1, in2, in3, in4);
%
...
%      OPTIONS:
%
%        out1 = variableSpy(in1, option1, option2, ...);
%
...
%         [xt,xtl] = variableSpy(xt,xtl,'multiple',{'tick','ticklabel'}, ...
...
DisplayNumber.Fields=5;
DisplayNumber.Columns=5;
DisplayNumber.Rows=10;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      elseif cmd(2)==4,
        ...
        while (~isempty(Options.Multiple) & (Dataset<=NDatasets)) | First,
          ...
          OtherZeroDims(Display.Dimensions(2))=logical(0);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      elseif cmd(2)==4,
        ...
        while (~isempty(Options.Multiple) & (Dataset<=NDatasets)) | First,
          ...
          OtherZeroDims=(size(TempVar)==0); % reset other dimensions that are empty back to emptiness
          OtherZeroDims(Display.Dimensions(1))=logical(0);
          if any(OtherZeroDims),
            [TempSubsIndex(end).subs{OtherZeroDims}]=deal([]);
            EditedData=LcSubsAsgn(EditedData,TempSubsIndex(1:(end-1)),LcSubsRef(EditedData,TempSubsIndex));
          end;
...
for j=1:Ind_HorzNumElement,
  ...
  Handles.Index.Label.Column(j)=uicontrol(...
    'parent',Handles.Figure, ...
    'units','pixels', ...
    'callback',['variableSpy(''MDEDIT-INTERNAL'',[0 ',num2str(j),'])'], ...
    'buttondownfcn','', ...
...
for i=1:Ind_VertNumElement,
  ...
  Handles.Index.Label.Row(i)=uicontrol(...
    'parent',Handles.Figure, ...
    'units','pixels', ...
    'callback',['variableSpy(''MDEDIT-INTERNAL'',[',str,' 0])'], ...
    'buttondownfcn','',...
...
set(Handles.Index.Scrollbar.Row,'value',0, ...
 'callback',['variableSpy(''MDEDIT-INTERNAL'',[-1 4])']);
...
set(Handles.Index.Scrollbar.Column,'value',0, ...
 'callback',['variableSpy(''MDEDIT-INTERNAL'',[-1 3])']);
...
for i=1:Ind_VertNumElement,
  for j=1:Ind_HorzNumElement,
    Handles.Index.Elements(i,j)=uicontrol(...
      ...
      'backgroundcolor',Color.Background, ...
      'enable','off', ...
      'callback',['variableSpy(''MDEDIT-INTERNAL'',[',num2str(i),' ',num2str(j),'])'], ...
      'userdata','2D Edit');
  end;
end;
...
for i=1:(Ind_VertNumElement+3),
  ...
  Handles.DimEdit.Label.Dim(i)=uicontrol(...
    ...
    'string',str, ...
    'enable','on', ...
    'buttondownfcn',['variableSpy(''MDEDIT-INTERNAL'',[',str,' 0])'], ...
    'callback',['variableSpy(''MDEDIT-INTERNAL'',[',str,' 0])'], ...
    'userdata','Dim Edit');
end;
...
for i=1:(Ind_VertNumElement+3),
  Handles.DimEdit.Dim(i)=uicontrol(...
    ...
    'enable','on', ...
    'callback',['variableSpy(''MDEDIT-INTERNAL'',[',num2str(i),' 1])'], ...
    'userdata','Dim Edit');
end;
...
Handles.TextEdit.Text=uicontrol(...
      ...
      'max',2, ...
      'enable','off', ...
      'callback',['variableSpy(''MDEDIT-INTERNAL'',[1 1])'], ...
      'userdata','0D Edit');
...
for i=1:Fld_NumFld,
  Handles.Field.Label.FieldName(i)=uicontrol(...
    ...
    'enable','off', ...
    'string','', ...
    'callback',['variableSpy(''MDEDIT-INTERNAL'',[-8 ',num2str(i),'])'], ...
    'userdata','Field');
...
set(Handles.Field.Scrollbar.Field,'value',0, ...
  'callback',['variableSpy(''MDEDIT-INTERNAL'',[-1 6])']);
...
Handles.General.Reset=uicontrol(...
  ...
  'enable','on', ...
  'string','reset', ...
  'callback',['variableSpy(''MDEDIT-INTERNAL'',[-1 2])'], ...
  'userdata','List');
...
Handles.General.List=uicontrol(...
  ...
  'horizontalalignment','left', ...
  'backgroundcolor',Color.Background, ...
  'callback',['variableSpy(''MDEDIT-INTERNAL'',[-1 5])'], ...
  'userdata','List');
...
seldim=uimenu(Handles.Menu.Dimen.Main, ...
   ...
   'enable','on', ...
   'callback',['variableSpy(''MDEDIT-INTERNAL'',[-4 1])']);
...
castr=uimenu(Handles.Menu.Char.Main, ...
   ...
   'enable','on', ...
   'callback',['variableSpy(''MDEDIT-INTERNAL'',[-5 1])']);
slstr=uimenu(Handles.Menu.Char.Main, ...
   'label','&single line strings', ...
   'enable','on', ...
   'callback',['variableSpy(''MDEDIT-INTERNAL'',[-5 2])']);
mlstr=uimenu(Handles.Menu.Char.Main, ...
   'label','&multiline string', ...
   'enable','on', ...
   'callback',['variableSpy(''MDEDIT-INTERNAL'',[-5 3])']);
...
Handles.Menu.Structure.NewField=uimenu(Handles.Menu.Structure.Main, ...
   'label','&new field', ...
   'enable','on', ...
   'callback',['variableSpy(''MDEDIT-INTERNAL'',[-7 1])']);
          ...
          if iscell(Display.Data) | isstruct(Display.Data) | ~isempty(Options.Multiple),
            if (isstruct(Display.Data) | ~isempty(Options.Multiple)) & all([Offset.Row+i Offset.Column+j]==Display.Element), 
              set(Handles.Index.Elements(i,j),'string',Str, ...
                           ...
                           'buttondownfcn',['variableSpy(''MDEDIT-INTERNAL'',[',num2str(i),' ',num2str(j),'])'], ...
          ...
          if isempty(TempElm),
            ...
            if isstruct(Display.Data),
              ...
              set(Handles.Field.Field(i),'string',Str, ...
                                  'buttondownfcn',['variableSpy(''MDEDIT-INTERNAL'',[-6 ',num2str(i),'])'], ...
                                  'callback','', ...
                                  'enable','inactive', ...
                                  'backgroundcolor',Color.Edit);
            ...
            %%-----------------------------------------------------------------
              ...
              set(Handles.Field.Field(i),'string',Str, ...
                                  'buttondownfcn',['variableSpy(''MDEDIT-INTERNAL'',[-6 ',num2str(i),'])'], ...
                                  'callback',['variableSpy(''MDEDIT-INTERNAL'',[-6 ',num2str(i),'])'], ...
                                  'enable',FieldEnable, ...
                                  'backgroundcolor',Color.Edit);
            end;
          end;
