function [S, stack_cnt] = metaTiffRead(filename, img_first, img_last, verbose)
% [S, nbimages] = tiffread;
% [S, nbimages] = tiffread(filename);
% [S, nbimages] = tiffread(filename, image);
% [S, nbimages] = tiffread(filename, first_image, last_image);
%
% Reads 16 and 8 bit uncompressed grayscale tiff files and stacks of multiple images,
...
% pixels values are stored in the field .data, in the native format (8 or 16 bit integer),
...
%
% Francois Nedelec, EMBL, 1999-2002. feedback/bugs to  nedelec@embl-heidelberg.de
...
% those used internally can be stored in TIF.
...
while (ifd_pos ~= 0)
    ...
    for i=1:stack_cnt
        ...
        if ( img_skip + 1 >= img_first )
            ...
            if ( stack_cnt > 1 && verbose), disp(sprintf('\b\b\b\b\b%4i', img_read)); end
...
function data = read_strips(TIF, strip_offsets, width, height, verbose)
