function [costMat,noLinkCost,nonlinkMarker,trackStartTime,trackEndTime,...
    indxMerge,numMerge,indxSplit,numSplit,errFlag] = costMatCloseGaps_D2(...
    trackedFeatInfo,trackStartTime,indxStart,trackEndTime,indxEnd,...
    costMatParams,gapCloseParam)
%COSTMATCLOSEGAPSDL provides a cost matrix for closing gaps (including merging and splitting) based on tracked feature statistics
%
%SYNOPSIS [costMat,noLinkCost,nonlinkMarker,trackStartTime,trackEndTime,...
%    indxMerge,numMerge,indxSplit,numSplit,errFlag] = costMatCloseGapsDL(...
%    trackedFeatInfo,trackStartTime,indxStart,trackEndTime,indxEnd,...
%    costMatParams,gapCloseParam)
%
...
%             .maxDist :     Maximum allowed distance (displacement bwt frames)  
%             .gapPenalty :  Gap penalty vector (should have same length as
%                            the timeWindow)
...
%displacement between two frames, and a field .gapPenalty, which is the
%penalty entered into the cost matrix for extended gaps between links
...
% in this version, the costMatParams also contain the maxDist field (as
% opposed to the global variable used earlier)
maxDistance = costMatParams.maxDist;
if length(maxDistance)<timeWindow
    error(['the maxDistance vector should have a length of at least ',num2str(timeWindow)]);
end
gapPenalty = costMatParams.gapPenalty;
if length(gapPenalty)<timeWindow
    error(['the gapPenalty vector should have a length of at least ',num2str(timeWindow)]);
end
...
%% ===== modification by Dinah START
% for pit tracking: use global value, and use for
%gap closing much smaller tolerance to prevent switch-over of neighbors
maxDispSq = (maxDistance.^2);
%% ===== modification by Dinah END
...
%% ===== modification by Dinah START
% for pit tracking: amplitude is not used;
%instead, introduce a slight penalty for increasing time gap
addConst = gapPenalty;
%% ===== modification by Dinah END
    ...
    for i=1:n %go over all ends
        ...
        if timeGap > 1 && timeGap <= timeWindow
            ...
            if dispSq < maxDispSq(timeGap) && ...
                ...
                %% === modification by Dinah START
                % for pit tracking: 
                % remove the amplitude difference criterion entirely, since
                % in TIRF, pit intensities can vary by any amount
                % original code (uncomment to replace)
