function [radialityParam,centerCoord] = radialArrangement(tracks,...
...
%                       tracks is. Calculated as the average angle between
%                       coming out of the center and the tracks directions.
...
for iTrack = 1 : numTracks
    ...
    dirAngle(iTrack,1) = acos(abs(rayDir * trackDir));
    
    %save also the number of points in this track
    dirAngle(iTrack,2) = length(~isnan(trackCoordX));

end %(for iTrack = 1 : numTracks)

%calculate the weighted average of the angles (give longer tracks more
%weight)
radialityParam = mean(dirAngle(:,1).*dirAngle(:,2))/mean(dirAngle(:,2));
