...
%          include weights
%
% c: jonas, 04/04,  Edited by EHarry April 2012, changed fitting line to
%                   nanmean (now on by default) and inlier std to the actual std of the inliers
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if fit
    ...
    % medianData = fminsearch(@(x)(nanmedian(abs(data-x))),nanmedian(data));
    medianData = fminsearch(@(x)(nanmedian((data-x).^2)),nanmedian(data));
else
...
if realDimensions == 1;
    ...
    if nargout > 1
        ...
        if nInlier > 4
            ...
            %             stdSample=sqrt(sum(res2(inlierIdx))/(nInlier));
            stdSample = nanstd(data(inlierIdx));
        else
            stdSample = NaN;
        end
    end
    ...
    if nargout > 1
        ...
        %         stdSample=sqrt(nansum(res2,dim)./(nInliers));
        stdSample = nanstd(data(inlierIdx));
    end
