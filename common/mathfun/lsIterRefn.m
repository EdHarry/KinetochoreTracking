function [lhsVec,errFlag] = lsIterRefn(lhsMat,rhsVec,tol)
...
if nargin ~= nargin('lsIterRefn')
    disp('--lsIterRefn: Incorrect number of input arguments!');
    errFlag  = 1;
    arParam = [];
    noiseSigma = [];
    return
...
if nCol >= nRow
    disp('--lsIterRefn: "lhsMat" should have more rows than columns!');
    errFlag = 1;
    lhsVec = [];
    return
end
...
if dummy2 ~= 1 || dummy1 ~= nRow
    disp('--lsIterRefn: "rhsVec" should be a column vector of length equal to number of rows in "lhsMat"!');
    errFlag = 1;
end
if tol <= 0
    disp('--lsIterRefn: "tol" should be a small, positive number!');
    errFlag = 1;
end
