function dataStruct = makiInitCoordDenoise(dataStruct, verbose, movieType, cutoffFix)
...
for t=goodTimes
    ...
    %%% denoiser
    denoised = haarDenoise(raw,0.00008);
    
    %%% remove offset again
   % offset = min(denoised(:));
   % denoised = denoised -  offset;
    ...
    FRAW = RAW .* FH;
    denoised_filtered = ifftshift( ifftn( FRAW ) );
    
    % Half-wave rectification
    denoised_filtered( denoised_filtered < 0 ) = 0;
...
%     error('bam')
    ...
    % find local maxima
    locMax = loc_max3Df(denoised_filtered,[3 3 3]);
 ...
 %   initCoordTmp = sortrows(initCoordTmp,-4);
     ...
     initCoordTmp = [locMax, filtered(locMaxIdx), noise(locMaxIdx), ...
        zeros(length(locMaxIdx),1)];
