...
for t=1:nTimePoints
    ...
    if initCoord(t).nSpots > 0
        ...
        try
            ...
            [dummy, dummy, inlierIdx, outlierIdx] = robustMean(meanNearestDist,2,[],1);
        catch
            inlierIdx = 1:initCoord(t).nSpots;
            outlierIdx = [];
        end
        ...
        if ~found
            subGraphs = findPlate( initCoord(t).allCoord(inlierIdx,1:probDim),2,30 );
            for g = 1:length(subGraphs)
                id = subGraphs(g).graph;
                subGraphs(g).graph = inlierIdx(id);
            end
        end
        indexC = 1;
        ...
        while ~found && ~endLoop
            ...
            index = subGraphs(indexC).graph;
            
            if length(index) < rankNNearestNeighbors+1
                break
            end
            
            inlierIdx = index;
            %outlierIdx = index(outlierIdx);
            allIdx = 1:initCoord(t).nSpots;
            outlierIdx = allIdx(~ismember(allIdx,index));
            
            
            if mod(t,5)==0 && verbose
                figure,hold on
                plot3(initCoord(t).allCoord(outlierIdx,1),initCoord(t).allCoord(outlierIdx,2),initCoord(t).allCoord(outlierIdx,3),'bx')
                plot3(initCoord(t).allCoord(inlierIdx,1),initCoord(t).allCoord(inlierIdx,2),initCoord(t).allCoord(inlierIdx,3),'r*')
            end
            
            
            % make an initial outlier detection
            outTemp = 1;
            while ~isempty(outTemp)
                
                dist = pdist(initCoord(t).allCoord(index,1:probDim));
                dist = squareform(dist);
                sortedDist = sort(dist);
                meanNearestDist = mean(sortedDist(2:rankNNearestNeighbors+1,:));
                ...
                % get kinetochores whose mean distance to the rankNNearestNeighbors
                % falls within the scatter of most of the kinetochores
                [dummy, dummy, inlierIdx, outTemp] = robustMean(meanNearestDist,2,[],1);
                
                inlierIdx = index(inlierIdx);
                %outlierIdx = index(outlierIdx);
                allIdx = 1:initCoord(t).nSpots;
                outlierIdx = allIdx(~ismember(allIdx,inlierIdx));
                index = inlierIdx;
            end
            
            if mod(t,5)==0 && verbose
                figure,hold on
                plot3(initCoord(t).allCoord(outlierIdx,1),initCoord(t).allCoord(outlierIdx,2),initCoord(t).allCoord(outlierIdx,3),'kx')
                plot3(initCoord(t).allCoord(inlierIdx,1),initCoord(t).allCoord(inlierIdx,2),initCoord(t).allCoord(inlierIdx,3),'g*')
            end
            ...
            if indexC > length(subGraphs)
                endLoop = 1;
            end
            
        end
    else % if initCoord(t).nSpots > 0
        planeFit(t).inlierIdx = [];
        planeFit(t).unalignedIdx = [];
        planeFit(t).planeOrigin = NaN(1,3);
        planeFit(t).eigenVectors = deal([]);
        planeFit(t).eigenValues = NaN(1,3);
        meanCoord(t,:) = deal(NaN);
        meanCoordFull(t,:) = deal(NaN);
        eigenVectors(:,:,t) = deal(NaN);
        eigenValues(t,:) = deal(NaN);
        
    end % if initCoord(t).nSpots > 0
...
% planeFit depends on dataProperites and initCoords
dependencies = struct('dataProperties',[],'initCoord',[]);
