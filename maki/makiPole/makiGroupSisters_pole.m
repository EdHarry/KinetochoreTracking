function dataStruct = makiGroupSisters_pole(dataStruct,verbose)
...
% % %                   the fields: "dataProperties", "initCoord", "planeFit_pole" &
% % %                   "tracks". Field "planeFit_pole" can be empty.
...
% % %                       eigenVector of planeFit_pole.eigenVectors
...
try
    trackStats = catStruct(3,'dataStruct.tracks_pole.seqOfEvents');
catch
...
framesWiPlane = [];
for t = 1 : nTimepoints
    if ~isempty(dataStruct.planeFit_pole) && ~isempty(dataStruct.planeFit_pole(t).planeVectors)
...
if ~isempty(dataStruct.planeFit_pole)
    framePhase = vertcat(dataStruct.planeFit_pole.phase);
else
...
if useAlignment == 1 && ~isempty(dataStruct.planeFit_pole)
    normals(framesWiPlane,:) = catStruct(2,'dataStruct.planeFit_pole.planeVectors(:,1)')';
end
...
%                 alpha = acos(abs(dot(distanceVectorN',commonNormals')));
                ...
                % go over common times, if either track in unaligned then
                % set the angle at that time to NaN so it does not contribute to the cost, EHarry April 2012
                % edit, do this by seeing if the spots are inliers rather
                % than not outliers, since there will always a list of
                % inliers
                count = 0;
                for iFrame = commonTimeNA'
                    % get featIdx position
                    count = count + 1;
                    iFeatPos = ctRowIdxNA(count);
                    jFeatPos = ctColIdxNA(count);
                    
                    % get spot indexes
                    iSpotIdx = dataStruct.tracks_pole(iIdx).tracksFeatIndxCG(iFeatPos);
                    jSpotIdx = dataStruct.tracks_pole(jIdx).tracksFeatIndxCG(jFeatPos);
                    
                    % get inliers
                    inliers = dataStruct.planeFit_pole(iFrame).inlierIdx;
                    
                    % test each spot idx, for each one if it is not zero test whether it is
                    % in the inlier list, if it is not then set alpha in
                    % this frame to NaN
                    
                    % iSpot
                    if iSpotIdx ~= 0
                        iSpotInlier = ismember(iSpotIdx,inliers);
                    else
                        iSpotInlier = true;
                    end
                    % jSpot
                    if jSpotIdx ~= 0
                        jSpotInlier = ismember(jSpotIdx,inliers);
                    else
                        jSpotInlier = true;
                    end
                    
                    % if either spot is not an inlier and the angle is
                    % greater than maxAngle then set alpha to 99% maxAngle
                    
                    if (~iSpotInlier || ~jSpotInlier) && alpha(count) > maxAngle
                        %                         alpha(count) = NaN;
                        alpha(count) = 0.99.*maxAngle;
                    end
                end
                
                % now, if all alpha are NaNs then set the angles to 99% of
                % maxAngle so that if gets through the max angle test
                % we don't need to worry about movies with no planes since
                % we're working in aligned coordinates (useAlignment will have been set to 0 if there are no planes)
                % EHarry April 2012
                
                if all(isnan(alpha))
                    alpha(1:end) = deal(0.99.*maxAngle);
                end
                ...
                if robust
                    meanAlpha = robustMean(alpha); % this is now robust, EHarry April 2012
                else
...
%% dependencies
% sisterList_pole depends on dataProperites, initCoords, planeFit, tracks,
% sisterList, updatedClass, frameAlignment, poles, poleReferenceFrame,
% planeFit_pole and tracks_pole
% dependencies = struct('dataProperties',[],'initCoord',[],'planeFit',[],'tracks',[],'sisterList',[],'updatedClass',[],'frameAlignment',[],'poles',[],'poleReferenceFrame',[],'planeFit_pole',[],'tracks_pole',[]);
...
% save into the first sisterList_pole
sisterList(1).dependencies = dependencies;


%% assign output to dataStruct

dataStruct.sisterList_pole = sisterList;
...
featIndx = dataStruct.tracks_pole(idx).tracksFeatIndxCG;

%get start time and end time of track
startTime = dataStruct.tracks_pole(idx).seqOfEvents(1,1);
endTime = dataStruct.tracks_pole(idx).seqOfEvents(2,1);
...
if ~isempty(dataStruct.poleReferenceFrame)
    for iFrame = 1 : lifeTime
        ...
        if featIndxTmp ~= 0
            coords(iFrame,1:3) = dataStruct.poleReferenceFrame(jFrame).poleCoords_cartisian(featIndxTmp,1:3);
            coordsStd(iFrame,1:3) = dataStruct.poleReferenceFrame(jFrame).poleCoords_cartisian(featIndxTmp,4:6);
        end
    end
elseif ~isempty(dataStruct.planeFit_pole)
    for iFrame = 1 : lifeTime
        jFrame = startTime + iFrame - 1;
        featIndxTmp = featIndx(iFrame);
        if featIndxTmp ~= 0
            coords(iFrame,1:3) = dataStruct.planeFit_pole(jFrame).rotatedCoord(featIndxTmp,1:3);
            coordsStd(iFrame,1:3) = dataStruct.planeFit_pole(jFrame).rotatedCoord(featIndxTmp,4:6);
        end
    end
else
    for iFrame = 1 : lifeTime
