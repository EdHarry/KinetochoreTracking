function sisterList = makiConstructAlignedSisters_plate2pole(dataStruct,removeNetDisp,...
    randomize)
% EHarry Jan 2012
...
poleReferenceFrame = dataStruct.poleReferenceFrame;
...
for iFrame = 1:frames
    if ~all(isnan(poleReferenceFrame(iFrame).poleCoords_cylindrical(:,1)))
        goodFrames = [goodFrames,iFrame];
    end
end

badFrames = setxor(1:frames,goodFrames);

if isempty(sisterList) || isempty(tracks) || isempty(frameAlignment)
    sisterList = [];
    return
end
...
for iSister = 1 : numSisters
    ...
    coords1(badFrames,:) = NaN;
    coords2(badFrames,:) = NaN;
    ...
    %     sisterList(iSister).coords1Aligned = coords1(goodFrames,:);
    %     sisterList(iSister).coords2Aligned = coords2(goodFrames,:);
    %     sisterList(iSister).distanceAligned = sqrt(sum((coords1(goodFrames,1:3) - coords2(goodFrames,1:3)).^2,2));
