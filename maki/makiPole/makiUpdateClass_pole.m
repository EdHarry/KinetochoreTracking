function dataStruct = makiUpdateClass_pole(dataStruct)
...
if isempty(planeFit) || isempty(tracks) || isempty(sisterList)
    dataStruct.updatedClass_pole = [];
    return
end

for i = 1:numFrames
    map(i).map = find(~isnan(dataStruct.poleReferenceFrame(i).poleCoords_cartisian(:,1)));
end
...
for iFrame = 1 : numFrames
    ...
    inlierIdx = intersect(inlierIdx,map(iFrame).map);
    if ~isempty(planeFit)
...
%% dependencies
% updatedClass_pole depends on dataProperites, initCoords, planeFit, tracks,
% sisterList, updatedClass, frameAlignment, poles, poleReferenceFrame,
% planeFit_pole, tracks_pole and sisterList_pole
% dependencies = struct('dataProperties',[],'initCoord',[],'planeFit',[],'tracks',[],'sisterList',[],'updatedClass',[],'frameAlignment',[],'poles',[],'poleReferenceFrame',[],'planeFit_pole',[],'tracks_pole',[],'sisterList_pole',[]);
...
% save into the first updatedClass_pole
updatedClass(1).dependencies = dependencies;



%% final output to dataStruct

dataStruct.updatedClass_pole = updatedClass;
