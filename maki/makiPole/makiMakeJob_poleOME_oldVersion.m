function job = makiMakeJob_poleOME_oldVersion(jobType,status,job,ask4input,movieType)
        ...
        for iJob = 1:nJobs
            ...
            if isempty(dataFile)
                ...
                switch movieType
                    ...
                    case 3
                        [~,metaData,newMovieNameAndPath] = readOMEAndSave( fullfile(rawMoviePath,rawMovieName) ); % save the movie and metaData as a matlab file for quick loading later
            ...
            if dataStruct.status(1) < 0
                ...
                try
                    ...
                    movie = readOMEMatFile( fullfile(dataStruct.rawMoviePath, dataStruct.rawMovieName),frameList,1 );
