function job = makiMakeJob_poleOME(jobType,status,job,ask4input,movieType,askDecon)
...
%                     end
            ...
            nJobs = length(job);
            fileList = cell(nJobs,2);
            for iJob = 1:nJobs
                switch movieType
                    case 1
                        list = searchFiles('dv$','(log)|(_PRJ)|(DIC)|(_REF)',job(iJob).dataStruct.rawMoviePath,1);
%                         if askDecon
%                             deconMovie(job(iJob).dataStruct.rawMoviePath,list);
%                         end
                    case 2
                        list = searchFiles('t1.STK$',[],job(iJob).dataStruct.rawMoviePath,1);
%                         if askDecon
%                             deconMovie(job(iJob).dataStruct.rawMoviePath,list);
%                         end
                    case 3
                        list = searchFiles('ome.tif',[],job(iJob).dataStruct.rawMoviePath,1);
%                         if askDecon
%                             deconMovie(job(iJob).dataStruct.rawMoviePath,list);
%                         end
                end
                fileList{iJob,1} = list{1};
                fileList{iJob,2} = list{2};
            end
        end
        % set up job file
        ...
        for iJob = 1:nJobs
            ...
            if movieType == 2 || any(findstr(rawMoviePath,projectName))
                ...
                listTemp = searchFiles('cmle.r3d',[],dataFilePath);
                if size(listTemp,1) == 1
                    decon = 1;
                else
                    decon = 0;
                end
            ...
            if isempty(dataFile)
                dataStruct = makiMakeDataStruct_pole(rawMovieName, ...
                ...
                switch movieType
                    ...
                    case 3
                        [~,metaData,newMovieNameAndPath] = readOMEAndSave( fullfile(rawMoviePath,rawMovieName), decon ); % save the movie and metaData as a matlab file for quick loading later
                        %[ movie,metaData ] = readOME(fullfile(rawMoviePath,rawMovieName));
                        dataStruct.movieHeader = convertOMEMetaDataToMakiHeader( metaData );
                        [~,name,ext] = fileparts(newMovieNameAndPath);
                        rawMovieName = [name,ext];
                        dataStruct.rawMovieName = rawMovieName;
                        %dataStruct.rawMovieName = squeeze(movie(:,:,:,:,1));
                        %clear movie
                end
                ...
                dataStruct.dataProperties.decon = decon;
            ...
            if dataStruct.status(1) < 0
                ...
                try
                    ...
                    movie = readOMEMatFile( fullfile(dataStruct.rawMoviePath, dataStruct.rawMovieName),frameList,1,dataStruct.dataProperties.decon );
                    projections = squeeze(max(movie,[],3));
                    % loop to get max-projections
                    %                     for f = 1:length(frameList)
                    %                         %img = r3dread(fullfile(rawMoviePath, rawMovieName),frameList(f),1);
                    %                         %img = dataStruct.rawMovieName(:,:,:,frameList(f));
                    %                         %img = movie(:,:,:,frameList(f),1);
            ...
            % assign pole coord system use
            if dataStruct.status(10) < 0
                dataStruct = getPoleCoordSystem(dataStruct,ask4input);
            end
...
maxDist_def = 1.5;
% minOverlap_def = 10;
% edit EHarry April 2012 use 40 frame sister overlap for 2s lapse movies
minOverlap_def = 40;
useAnaphase_def = 1;
%robust_def = 0;
% edit EHarry April 2012 use robust stats as default
robust_def = 1;
...
function dataStruct = getPoleCoordSystem(dataStruct,ask4input)
...
if isfield(dataPropertiesTmp,'tracksParam') %if tracksParam have been assigned previously
    tracksParamTmp = dataPropertiesTmp.tracksParam;
    if isfield(tracksParamTmp,'pole')
        poleTmp = tracksParamTmp.pole;
    else
        poleTmp = pole_def;
    end
    ...
    if ask4input
        out = inputdlg({'Eventualy try and use pole based coordinates? (1 yes, 0 no)'},'Use Pole Coordinates?',1,{'1'});
        if isempty(out)
            error('input aborted')
        else
            poleTmp = str2double(out{1});
        end
    end
    
    dataStruct.dataProperties.tracksParam.pole = poleTmp;
