...
if plate
    ...
    c=1;
    maxLag = floor(size(plateC(1).sisterList(1).coords1,1)./2); % maxLag for autoCorr
    ...
    gammaPole = autoCorr(trajPole,maxLag);
    ...
    gammaPlate = autoCorr(trajPlate,maxLag);
...
if verbose
    ...
    if plate
        ...
        hold on
        plot(lag.*(0:maxLag),analysis.poleVSPlateSisterCentres.autoCorr.plate(:,1),'k');
        myErrorbar(lag.*(0:maxLag),analysis.poleVSPlateSisterCentres.autoCorr.plate(:,1),analysis.poleVSPlateSisterCentres.autoCorr.plate(:,2));
        plot(lag.*(0:maxLag),analysis.poleVSPlateSisterCentres.autoCorr.pole(:,1),'g');
        myErrorbar(lag.*(0:maxLag),analysis.poleVSPlateSisterCentres.autoCorr.pole(:,1),analysis.poleVSPlateSisterCentres.autoCorr.pole(:,2));
        xlabel('time lag');
        ylabel('autoCorr');
