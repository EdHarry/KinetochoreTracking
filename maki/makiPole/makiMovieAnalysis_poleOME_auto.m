function makiMovieAnalysis_poleOME_auto(serverType,job,movieType)
...
while ~done
    ...
    for iJob = 1:nJobs
        ...
        try
            ...
            %---------------- KT model tracking -------------------
            %++++++++++++++++++++++++++++++++++++++++++++++++++++++
            if any(jobs2do == 7) && any(jobs2do == 8) && any(jobs2do == 9) 
                
                %write current job to log files
                fprintf(1,'%s : KT model tracking for %s\n',...
                    nowString,job(iJob).dataStruct.projectName);
                %fprintf(generalLog,'%s : align frames\n',nowString);
                fprintf(individualLog,'%s : KT model tracking\n', nowString);
                
                % KT model tracking
                job(iJob).dataStruct = KTPairTracking(job(iJob).dataStruct);
                
                %save dataStruct. Do not overwrite older fields
                [success, job(iJob).dataStruct ] = makiSaveDataFile(serverType,job(iJob).dataStruct,{'initCoord','planeFit','tracks','sisterList','updatedClass','frameAlignment'});
                ...
                if success
                    ...
                    dataPropertiesName = job(iJob).dataStruct.dataPropertiesName;
                    dataPropertiesV = getVersion(dataPropertiesName);
                    initCoordName = job(iJob).dataStruct.initCoordName;
                    initCoordV = getVersion(initCoordName);
                    planeFitName = job(iJob).dataStruct.planeFitName;
                    planeFitV = getVersion(planeFitName);
                    tracksName = job(iJob).dataStruct.tracksName;
                    ...
                    if ~isempty(job(iJob).dataStruct.frameAlignment)
                        ...
                        if ~isempty(job(iJob).dataStruct.sisterList)
                            job(iJob).dataStruct.frameAlignment(1).dependencies.sisterList = sisterListV;
                        end
                        if ~isempty(job(iJob).dataStruct.tracks)
                            job(iJob).dataStruct.frameAlignment(1).dependencies.tracks = tracksV;
                        end
                        if ~isempty(job(iJob).dataStruct.planeFit)
                        job(iJob).dataStruct.frameAlignment(1).dependencies.planeFit = planeFitV;
                        end
                        job(iJob).dataStruct.frameAlignment(1).dependencies.initCoord = initCoordV;
                        job(iJob).dataStruct.frameAlignment(1).dependencies.dataProperties = dataPropertiesV;
                    end
                    ...
                    if ~isempty(job(iJob).dataStruct.updatedClass)
                        ...
                        if ~isempty(job(iJob).dataStruct.tracks)
                            job(iJob).dataStruct.updatedClass(1).dependencies.tracks = tracksV;
                        end
                        if ~isempty(job(iJob).dataStruct.planeFit)
                            job(iJob).dataStruct.updatedClass(1).dependencies.planeFit = planeFitV;
                        end
                        job(iJob).dataStruct.updatedClass(1).dependencies.initCoord = initCoordV;
                        job(iJob).dataStruct.updatedClass(1).dependencies.dataProperties = dataPropertiesV;
                    end
                    
                     % update for sisterList
                    if ~isempty(job(iJob).dataStruct.sisterList)
                        if ~isempty(job(iJob).dataStruct.tracks)
                            job(iJob).dataStruct.sisterList(1).dependencies.tracks = tracksV;
                        end
                        if ~isempty(job(iJob).dataStruct.planeFit)
                            job(iJob).dataStruct.sisterList(1).dependencies.planeFit = planeFitV;
                        end
                        job(iJob).dataStruct.sisterList(1).dependencies.initCoord = initCoordV;
                        job(iJob).dataStruct.sisterList(1).dependencies.dataProperties = dataPropertiesV;
                    end
                    
                    % update for tracks
                    if ~isempty(job(iJob).dataStruct.tracks)
                        if ~isempty(job(iJob).dataStruct.planeFit)
                            job(iJob).dataStruct.tracks(1).dependencies.planeFit = planeFitV;
                        end
                        job(iJob).dataStruct.tracks(1).dependencies.initCoord = initCoordV;
                        job(iJob).dataStruct.tracks(1).dependencies.dataProperties = dataPropertiesV;
                    end
                    
                    % update for planeFit
                    if ~isempty(job(iJob).dataStruct.planeFit)
                        job(iJob).dataStruct.planeFit(1).dependencies.initCoord = initCoordV;
                        job(iJob).dataStruct.planeFit(1).dependencies.dataProperties = dataPropertiesV;
                    end
                    
                    % update for initCoord
                    if ~isempty(job(iJob).dataStruct.initCoord)
                        job(iJob).dataStruct.initCoord(1).dependencies.dataProperties = dataPropertiesV;
                    end
                    
                    % new save, no need to secure-save and structures this
                    ...
                    if ~success2
                        error('unable to secure-save initCoord, planeFit, tracks, sisterList, updatedClass or frameAlignment after KT model tracking');
                    end
                else
                    error('unable to secure-save initCoord, planeFit, tracks, sisterList, updatedClass or frameAlignment after KT model tracking');
                end
