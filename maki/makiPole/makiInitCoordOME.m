function dataStruct = makiInitCoordOME(dataStruct, verbose, movieType, cutoffFix)

% EHarry Jan 2012
% new, EHarry April 2012, version control, tracking subroutines wil now
% record the version numbers of thier dependencies so correct information
% can be loaded incase of multiple analysies
...
if isfield(dataStruct.dataProperties,'decon')
    decon = dataStruct.dataProperties.decon; % flag for reading deconvolved data
else
    decon = 0;
end
...
for t=goodTimes
    ...
    elseif isempty(rawMovie)
        ...
        switch movieType
            ...
            case 3
                raw = readOMEMatFile(rawMovie,t,1,decon);
                cropInfo = dataProperties.crop; %cropping information
                if ~isempty(cropInfo)
                    cropInfo = cropInfo(:,1:2);
                    cropInfo(1,:) = max(cropInfo(1,:),[1 1]); %defaults for min
