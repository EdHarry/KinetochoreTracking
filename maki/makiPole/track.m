function track( jobVector, askDecon, basePath )
% EHarry Feb 2012
...
if nargin < 3 || isempty(basePath)
    ...
    while ~strcmp(strResponse,'done');
        if ~isempty(basePath)
            basePath{end+1} = uigetdir(basePath{end});
        else
    ...
    for iJob = jobs2Test
        ...
        if isempty(dataFile_path)
            ...
            try
                ...
                toDel = [toDel iJob]; % remove jobs that can't have dataStruct's read
            end
        end
