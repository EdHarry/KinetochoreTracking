function dataStruct = makiAlignFrames_pole_oldVersion(dataStruct)
...
%                    fields "dataProperties", "initCoord", "poleReferenceFrame", 
%                    "tracks_pole" & "updatedClass_pole". Field "poleReferenceFrame" can be
%                    empty.
...
%
%eharry, Dec 2011, based on makiAlignFrames.m by KJaqaman
...
[tracksInfo,tracksIndx] = convStruct2MatNoMS(dataStruct.tracks_pole);
...
for iFrame = 1 : numFrames
    outIndx = [dataStruct.updatedClass_pole(iFrame).unalignedIdx dataStruct.updatedClass_pole(iFrame).laggingIdx];
    for iFeat = outIndx
...
for iFrame = framesOK
    if isempty(dataStruct.poleReferenceFrame) || isempty(dataStruct.poleReferenceFrame(iFrame).planeVectors)
        coordSystem(:,:,iFrame) = eye(3);
    else
        coordSystem(:,:,iFrame) = dataStruct.poleReferenceFrame(iFrame).planeVectors;
    end
end    
...
dataStruct.frameAlignment_pole = frameAlignment;
