function [ alignedTracks , atLeastOnePlaneFit ] = makiConstructAlignedTracks( dataStruct )
...
if (isempty(planeFit) && isempty(frameAlignment)) || isempty(tracks) % end if data is missing
    alignedTracks = [];
    atLeastOnePlaneFit = [];
    return
end
...
if isempty(planeFit) || isempty(cat(1,planeFit.planeVectors)) % look for at least one planeFit by looking for at least one non-empty list of planeVectors
