function master_track_poleOME_setupOnly(job,jobVector,updateJob,askDecon)
...
if updateJob || isempty(job)
    makiMakeJob_poleOME('MCAINSH',jobVector,job,0,[],askDecon); %makimakejobed returns the basepath aswell as the job struct, it also automatically uses default tracking parameters, change last 0 to 1 to make fuction ask for parameters
end
