function [eflag,poleCoord,pole1Track,pole2Track] = cylindericalTransformGammaTubulin(planeFit,tracks,sisterList,minFrames)
%poles find poles
...
%%% pole cirteria
maxDistance=8; % max pole search distance (microns)
minDistance=5; % min pole search distacne (microns)
maxAngle=35*pi()/180; % max pole search angle (degrees in radians)
%minFrames=20; % min no. of frames a pole track must exist
%%%
    ...
    for j=1:length(sisterList) % convert to cylindrical coords
        if ~isempty(sisterList(j).coords1)
            if ~isnan(sisterList(j).coords1(iTime,1)) % check sister coords exist for timepoint
                ...
                z(iCyl,iTime) = polePoleDistance(iTime)/2 - z(iCyl,iTime);
                z(iCyl+1,iTime) = polePoleDistance(iTime)/2 - z(iCyl+1,iTime);
                %%%%%%%%
                %%%%%%%%
