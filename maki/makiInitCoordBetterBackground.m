function dataStruct = makiInitCoordBetterBackground(dataStruct, verbose, movieType, cutoffFix)
...
if ~isfield(dataProperties,'betterBackground')
    if dataObject
        ...
        betterBackground = true; %%% EH - changed false to true (think this was an original bug)
    % betterBackground = false; %%% EH - changed false to true (think this was an original bug)
    end
end
...
%%% EH - need much smoother background for the denoised image
backgroundFilterParms = dataProperties.FT_SIGMA * 40;
%%%
...
for t=goodTimes
    ...
    %%% EH - haar filter, gamma of 0.001 chosen by inspection
    raw=haarDenoise(raw,0.001);
    %%%
    ...
    if betterBackground
       ...
       %sigMask = raw>1.09*background;
        ...
        %%% EH - change: fill in binary mask holes here
        ...
        %%% EH - change: don't need to fill holes here anymore 
        ...
        %%% EH - need to turn NaNs in background back to zeros
...
cutoffCol2=[];
 ...
 %%% EH - changing for betterbackground to go with amp cutoff first
 ...
 %%%   
            ...
            if nn(1) > 1
                cutoffIdx = 1; %EH - filter by amp then by noise
               cutoffCol = 4;
                cutoff3 = splitModes(allCoord((allCoord(:,4)>cutoff(1)),8));
                cutoff2 = splitModes(allCoord((allCoord(:,4)>cutoff(1)),7));
                if ~isempty(cutoff3)
                 cutoff(3) = cutoff3;
                else
                 cutoff(3) = NaN;
              ...
              nn(3) = sum(allCoord((allCoord(:,4)>cutoff(1)),8)>cutoff(3))/minGood;
              nn(2) = sum(allCoord((allCoord(:,4)>cutoff(1)),7)>cutoff(2))/minGood;
              if nn(3) > 1
                cutoffIdx2 = 3;
                cutoffCol2 = 8;
              elseif nn(2) > 1
...
for t=goodTimes
    ...
    if ~isempty(cutoffCol2)
      goodIdxL2 =  initCoordRaw{t}(:,cutoffCol2) > cutoff(cutoffIdx2);
      goodIdxL = logical(goodIdxL.*goodIdxL2);
    end
