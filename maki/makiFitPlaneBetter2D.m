function dataStruct = makiFitPlaneBetter2D(dataStruct,verbose)
...
if use2D
    ...
    for t = 1 : nTimePoints
        ...
        tmpCoord = initCoord(t).allCoord(planeFit(t).inlierIdx,:);
        tmpCoord(:,1:3) = tmpCoord(:,1:3) - ...
            repmat(frameOrigin(t,:),length(planeFit(t).inlierIdx),1);
        e_plane = calcPlaneVectors(eigenVectors(:,eigenVecAssign(normalIndx,t),goodFrames(t))); % first approx. straight down in z
        planeCoord = ...
            (inv(e_plane)*...
            (tmpCoord(:,1:3))')';
        [~,inlierIdx] = detectOutliers(planeCoord(:,1),1); % find initial inliers
        tmpCoord = tmpCoord(inlierIdx,:);
        
        b = fminsearch(@(b)rotatedNormalVarRotated(eigenVectors(:,:,goodFrames(t)),eigenVecAssign(normalIndx,t),tmpCoord,b),0);
        
        rotationVector = qRotate(eigenVectors(:,eigenVecAssign(normalIndx,t),goodFrames(t)),[0,0,-1],pi()/2);
        eigenVectors(:,eigenVecAssign(normalIndx,t),goodFrames(t)) = qRotate(eigenVectors(:,eigenVecAssign(normalIndx,t),goodFrames(t)),rotationVector,b); % rotate the normal
        eigenVectors(:,3,goodFrames(t)) = qRotate(eigenVectors(:,3,goodFrames(t)),rotationVector,b); % rotate the 3rd eigenvector, no need to rotate the 2nd since this is in the xy plane
        
        %         q  = [cos(b),0,-sin(b),0;0,1,0,0;sin(b),0,cos(b),0;0,0,0,1];
        %         temp = q*[eigenVectors(:,1,goodFrames(t));1];
        %         eigenVectors(:,1,goodFrames(t)) = temp(1:3);
        %         temp = q*[eigenVectors(:,2,goodFrames(t));1];
        %         eigenVectors(:,2,goodFrames(t)) = temp(1:3);
        %         temp = q*[eigenVectors(:,3,goodFrames(t));1];
        %         eigenVectors(:,3,goodFrames(t)) = temp(1:3);
        
        planeFit(goodFrames(t)).eigenVectors = eigenVectors(:,:,goodFrames(t));
        planeFit(goodFrames(t)).eigenValues = [planeFit(goodFrames(t)).eigenValues NaN];
        
        % v = rotatedNormalVarRotated(eigenVectors(:,:,goodFrames(t)),eigenVecAssign(normalIndx,t),tmpCoord,b);
        %         e_plane = calcPlaneVectors(eigenVectors(:,eigenVecAssign(normalIndx,t),goodFrames(t)));
        %         rotationMat = inv(e_plane); %rotation matrix
        %         tmpCoord(:,1:3) = (rotationMat*(tmpCoord(:,1:3))')';
...
function positionVar = rotatedNormalVar(eigenVectors,tmpCoord)
% variance of rotated cordinates along the normal
e_plane = calcPlaneVectors(eigenVectors);
rotationMat = inv(e_plane); %rotation matrix
tmpCoord(:,1:3) = (rotationMat*(tmpCoord(:,1:3))')';
positionVar = sum(tmpCoord(:,1));

function positionVarRotated = rotatedNormalVarRotated(eigenVectors,evAs,tmpCoord,b)
%q  = [cos(b),0,-sin(b),0;0,1,0,0;sin(b),0,cos(b),0;0,0,0,1];
%eigenVectors = q*[eigenVectors(:,evAs);1];
% first calculate rotation vector by rotating the normal around -z by
% pi()/2
rotationVector = qRotate(eigenVectors(:,evAs),[0,0,-1],pi()/2);
% then rotate the normal by this vector by the angle b
eigenVectors = qRotate(eigenVectors(:,evAs),rotationVector,b);
positionVarRotated = rotatedNormalVar(eigenVectors,tmpCoord);

function vectorOut = qRotate(vectorIn,rVector,a)
% rotate vector by angle a around rVector using quanterions
planeUnit = rVector ./ norm(rVector);
q0 = cos(a/2);
q1 = sin(a/2)*planeUnit(1);
q2 = sin(a/2)*planeUnit(2);
q3 = sin(a/2)*planeUnit(3);
...
vectorOut = q*vectorIn; % rotate

vectorOut = vectorOut ./ norm(vectorOut); % norm
%% old stuff
