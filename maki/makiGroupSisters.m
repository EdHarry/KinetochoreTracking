...
% DATE: 16-Jul-2007
%
%   EHarry April 2012, edit to 1) use robust statistics and 2) not add the
%   angle cost to unaligned pairs so we find the unaligned chromosomes
%   as well
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            if (useAnaphase && (sumSigns==0)) || ~useAnaphase
                ...
                if useAlignment
                    for iFrame = commonTimeNA'
                        ...
                        % get spot indexes
                        iSpotIdx = dataStruct.tracks(iIdx).tracksFeatIndxCG(iFeatPos);
                        jSpotIdx = dataStruct.tracks(jIdx).tracksFeatIndxCG(jFeatPos);
                        
                        % get inliers
                        inliers = dataStruct.planeFit(iFrame).inlierIdx;
...
% sisterList depends on dataProperites, initCoords, planeFit and tracks
% dependencies = struct('dataProperties',[],'initCoord',[],'planeFit',[],'tracks',[]);
...
% only add dependencies on planeFit if it is present
if ~isempty(dataStruct.planeFit)
    dependencies.planeFit = planeFitV;
end
dependencies.tracks = tracksV;

% save into the first sisterList
