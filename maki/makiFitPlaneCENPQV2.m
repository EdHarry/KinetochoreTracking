function dataStruct = makiFitPlaneCENPQV2(dataStruct,verbose)
...
for t=1:nTimePoints
    ...
    pset = powerset(num2cell(inlierIdx)); % all possible subsets of inliers
    pset = pset(2:end-1); % first and last are the complete set and empty set, which are trivial
    
    c=1;
    for i = 1:length(pset)
        if length(pset{i}) > minNumInliers-1
            psetTemp{c} = pset{i};
            c=c+1;
        end
    end
    ...
    while loop && loopIdx <= length(pset)
        ...
        if( (eigenValues(t,maxIndx)/mean(eigenValues(t,setdiff(1:probDim,maxIndx))) ...
            ...
            loopIdx = loopIdx + 1;
            try
                inlierIdx = cell2mat(pset{loopIdx}); % delete one of the inliers
            end
        end
    ...
    planeFit(t).unalignedIdx = allIdx;
