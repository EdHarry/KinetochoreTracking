function [conn,spots,nSpotSum,pixelSize,nTimepoints] = makiShowImarisICEOME_forTrajDataVisu(dataStruct,select,movieType,imarisLoad,decon,c2Load,conn)
...
elseif length(select) < 3
    select = [select 1 1 1 1 1];
elseif length(select) < 4
    select = [select 1 1 1 1];
elseif length(select) < 5
    select = [select 1 1 1];
elseif length(select) < 6
    select = [select 1 1];
elseif length(select) < 7
    select = [select 1];
end
...
if nargin < 4 || isempty(imarisLoad)
    imarisLoad = 1;
end

if nargin < 5 || isempty(decon)
    decon = 1;
end
...
if isempty(conn)
    conn = IceImarisConnector; % start the connector
else
    conn = IceImarisConnector(conn);
end

% open imaris if it's not already open
if ~conn.isAlive
    sucssess = 0; % flag for sussesfully starting imaris
    tries = 0; % index count
    
    while ~sucssess && tries < 100
        try
            sucssess = conn.startImaris;
        end
        tries = tries + 1;
    end
    
    if ~sucssess
        error('Could not start imaris')
    end
end

imarisApplication = conn.mImarisApplication; % get the app handle
...
switch movieType
    ...
    case 3
        if ~imarisLoad
            list = searchFiles('.ome.tif',[],dataStruct.rawMoviePath); % rawMoveName is the actual movie, so need to search the directory for the .ome.tif, assume that there is only one .ome.tif in there, as there should be   EH
            imarisApplication.FileOpen(...
                fullfile(list{2},list{1}),'reader=''OmeTiff''');
        else
            makiImarisLoadMovie(imarisApplication,dataStruct,decon,c2Load);
        end
end
...
if select(3) && ~isempty(dataStruct.planeFit)
    ...
    if ~isempty(inlierIdx)
        ...
        imaSpotsInlier.Set(spots(inlierIdx,1:3),(spots(inlierIdx,4))',0.4.*(spots(inlierIdx,5))');
        ...
        imarisApplication.GetSurpassSelection.SetColorRGBA(convertRGB2RGBA(0.8,0.8,0.8,0));
        imarisApplication.GetSurpassSelection.SetVisible(1);
    end
    
    %plot unaligned kinetochores
    if ~isempty(unalignedIdx)
        imaSpotsUnaligned = imarisApplication.GetFactory.CreateSpots;
        imaSpotsUnaligned.Set(spots(unalignedIdx,1:3),(spots(unalignedIdx,4))',0.4.*(spots(unalignedIdx,5))');
    ...
    if ~isempty(laggingIdx)
        ...
        imaSpotsLagging.Set(spots(laggingIdx,1:3),(spots(laggingIdx,4))',0.4.*(spots(laggingIdx,5))');
...
if select(4) && ~isempty(dataStruct.poles)
    
    poleTracks = imarisApplication.GetFactory.CreateDataContainer;
    
    %get tracks from dataStruct
    %tracksFinal = dataStruct.tracks;
    
    %tracksFinal = tracksFinal([dataStruct.poles.pole1TrackIdx,dataStruct.poles.pole2TrackIdx]); % keep only the poles
    
    tracksFinal = [dataStruct.poles.pole1Track,dataStruct.poles.pole2Track];
    ...
    for iTrack = 1 : numTracks
        ...
        imaTracks.SetColorRGBA(convertRGB2RGBA(0,1,1,0));
        poleTracks.AddChild(imaTracks,-1);
    ...
    %give names to groups of tracks
    imarisApplication.GetSurpassScene.AddChild(poleTracks,-1);
    imarisApplication.SetSurpassSelection(imarisApplication.GetSurpassScene.GetChild(imarisApplication.GetSurpassScene.GetNumberOfChildren-1));
    imarisApplication.GetSurpassSelection.SetName('Spindle Poles');
    imarisApplication.GetSurpassSelection.SetVisible(1);
...
if select(5) && ~isempty(dataStruct.tracks_pole)
    % not debugged yet
    ...
    %get tracks from dataStruct
    tracksFinal = dataStruct.tracks_pole;
    ...
    imarisApplication.GetSurpassScene.AddChild(imaTrackGroup90to100,-1);
    imarisApplication.SetSurpassSelection(imarisApplication.GetSurpassScene.GetChild(imarisApplication.GetSurpassScene.GetNumberOfChildren-1));
    imarisApplication.GetSurpassSelection.SetName(['tracks_pole length 90-100% (' ...
        num2str(numTracks90to100) ')']);
    imarisApplication.GetSurpassSelection.SetVisible(0);
    
    imarisApplication.GetSurpassScene.AddChild(imaTrackGroup50to90,-1);
    imarisApplication.SetSurpassSelection(imarisApplication.GetSurpassScene.GetChild(imarisApplication.GetSurpassScene.GetNumberOfChildren-1));
    imarisApplication.GetSurpassSelection.SetName(['tracks_pole length 50-90% (' ...
        num2str(numTracks50to90) ')']);
    imarisApplication.GetSurpassSelection.SetVisible(0);
    
    imarisApplication.GetSurpassScene.AddChild(imaTrackGroup0to50,-1);
    imarisApplication.SetSurpassSelection(imarisApplication.GetSurpassScene.GetChild(imarisApplication.GetSurpassScene.GetNumberOfChildren-1));
    imarisApplication.GetSurpassSelection.SetName(['tracks_pole length 0-50% (' ...
        num2str(numTracks0to50) ')']);
    imarisApplication.GetSurpassSelection.SetVisible(0);
...
if select(6) && ~isempty(dataStruct.sisterList_pole) && ~isempty(dataStruct.sisterList_pole(1).trackPairs)
    if interface62
        % implementation for Imaris 6.2
        ...
        %get sister list from dataStruct
        sisterList = dataStruct.sisterList_pole;
        
        %determine number of pairs
        numPairs = length(dataStruct.sisterList_pole);
        ...
        imarisApplication.GetSurpassScene.AddChild(imaTrackAllSisters,-1);
        imarisApplication.SetSurpassSelection(imarisApplication.GetSurpassScene.GetChild(imarisApplication.GetSurpassScene.GetNumberOfChildren-1));
        imarisApplication.GetSurpassSelection.SetName(['sister pairs_pole (' num2str(numPairs) ')']);
        %imarisApplication.GetSurpassSelection.SetVisible(0);
...
if select(7) && ~isempty(dataStruct.poleReferenceFrame) && ~isempty(dataStruct.poles) && ~isempty(dataStruct.planeFit_pole)
    ...
    for i = 1:length(dataStruct.poles.pole1Track)
        pole1Complete_temp(:,:,i) = getTrackCoords(dataStruct.poles.pole1Track(i) , nTimepoints);
    end
    
    for i = 1:length(dataStruct.poles.pole2Track)
        pole2Complete_temp(:,:,i) = getTrackCoords(dataStruct.poles.pole2Track(i) , nTimepoints);
    end
    ...
    %get plane fit results and updated classification
    planeFit = dataStruct.planeFit_pole;
    updatedClass = dataStruct.updatedClass_pole;
    poleR = dataStruct.poleReferenceFrame;
    ...
    if ~isempty(inlierIdx)
        ...
        imarisApplication.GetSurpassScene.AddChild(imaSpotsInlier,-1);
        imarisApplication.SetSurpassSelection(imarisApplication.GetSurpassScene.GetChild(imarisApplication.GetSurpassScene.GetNumberOfChildren-1));
        imarisApplication.GetSurpassSelection.SetName(['Inlier spots_pole (avg: ' num2str(length(inlierIdx)/nTimepoints) ' / frame)']);
        imarisApplication.GetSurpassSelection.SetColorRGBA(convertRGB2RGBA(0.8,0.8,0.8,0));
        imarisApplication.GetSurpassSelection.SetVisible(0);
    end
    ...
    if ~isempty(unalignedIdx)
        ...
        imarisApplication.GetSurpassScene.AddChild(imaSpotsUnaligned,-1);
        imarisApplication.SetSurpassSelection(imarisApplication.GetSurpassScene.GetChild(imarisApplication.GetSurpassScene.GetNumberOfChildren-1));
        imarisApplication.GetSurpassSelection.SetName(['Unaligned spots_pole (avg: ' num2str(length(unalignedIdx)/nTimepoints) ' / frame)']);
        imarisApplication.GetSurpassSelection.SetColorRGBA(convertRGB2RGBA(1,0,0,0));
        imarisApplication.GetSurpassSelection.SetVisible(0);
    end
    ...
    if ~isempty(laggingIdx)
        ...
        imarisApplication.GetSurpassScene.AddChild(imaSpotsLagging,-1);
        imarisApplication.SetSurpassSelection(imarisApplication.GetSurpassScene.GetChild(imarisApplication.GetSurpassScene.GetNumberOfChildren-1));
        imarisApplication.GetSurpassSelection.SetName(['Lagging spots_pole (avg: ' num2str(length(laggingIdx)/nTimepoints) ' / frame)']);
        imarisApplication.GetSurpassSelection.SetColorRGBA(convertRGB2RGBA(0,0,1,0));
        imarisApplication.GetSurpassSelection.SetVisible(0);
    end
    
    %initialize variables storing grid spots
    spotsGridPole = []; %planes from eigenvectors and eigenvalues
    %spotsGridInter = []; %planes from interpolation
    
    %go over all time points
    for iTime = 1 : nTimepoints
        
        if ~isempty(dataStruct.frameAlignment_pole)
            planeVectors = dataStruct.frameAlignment_pole(iTime).coordSystem;
        else
            planeVectors = poleR(iTime).planeVectors;
        end
        ...
        if ~isempty(planeVectors)
            ...
            %get the origin of the plane to be plotted
            planeOrigin = (((poleR(iTime).origin ./ pixelSize) - 1.5 + crop(1,:)) .* pixelSize) + zeroOffset;
            ...
            vec1 = 0.2*dataStruct.poleReferenceFrame(iTime).planeVectors(:,1);
            vec2 = 0.2*dataStruct.poleReferenceFrame(iTime).planeVectors(:,2);
            vec3 = 0.2*dataStruct.poleReferenceFrame(iTime).planeVectors(:,3);
            if movieType == 2
                vec1(2) = -vec1(2);
                vec2(2) = -vec2(2);
                vec3(2) = -vec3(2);
            ...
            %add grid information to the information on all grids
            %if planeFit(iTime).planeVectorClassifier == 1
            spotsGridPole = [spotsGridPole; spotsGrid];
            %else
            %   spotsGridInter = [spotsGridInter; spotsGrid];
            %end
    ...
    %add grids to surpassScene
    if ~isempty(spotsGridPole)
        imaSpotsGrid = imarisApplication.GetFactory.CreateSpots;
        imaSpotsGrid.Set(spotsGridPole(:,1:3),(spotsGridPole(:,4))',(spotsGridPole(:,5))');
        imarisApplication.GetSurpassScene.AddChild(imaSpotsGrid,-1);
        imarisApplication.SetSurpassSelection(imarisApplication.GetSurpassScene.GetChild(imarisApplication.GetSurpassScene.GetNumberOfChildren-1));
        imarisApplication.GetSurpassSelection.SetColorRGBA(convertRGB2RGBA(0,1,1,0));
        %imarisApplication.GetSurpassSelection.SetVisible(0);
        %         if isempty(aStartFrame)
        %             anaText = [];
        %         else
        %             anaText = [' - A: TP ' num2str(aStartFrame)];
        %         end
        imarisApplication.GetSurpassSelection.SetName('Plane fits_pole');
    end
    %     if ~isempty(spotsGridInter)
    %         imaSpotsGrid2 = imarisApplication.GetFactory.CreateSpots;
    %         imaSpotsGrid2.Set(spotsGridInter(:,1:3),(spotsGridInter(:,4))',(spotsGridInter(:,5))');
    ...
    spotsLine = []; % line between poles
    
    for iTime = 1:nTimepoints
        
        if ~isnan(pole1Complete(iTime,1)) && ~isnan(pole2Complete(iTime,1))
            
            line = make3dLine( (((pole1Complete(iTime,:) ./ pixelSize) - 1.5 + crop(1,:)) .* pixelSize) + zeroOffset  ,(((pole2Complete(iTime,:) ./ pixelSize) - 1.5 + crop(1,:)) .* pixelSize) + zeroOffset  , 50 );
            
            line = [line (iTime-1)*ones(size(line,1),1) pixelSize(1)*0.5*ones(size(line,1),1)];
            
            spotsLine = [spotsLine;line];
            
        end
        
    end
    
    if ~isempty(spotsLine)
        imaSpotsGrid = imarisApplication.GetFactory.CreateSpots;
        imaSpotsGrid.Set(spotsLine(:,1:3),(spotsLine(:,4))',(spotsLine(:,5))');
        imarisApplication.GetSurpassScene.AddChild(imaSpotsGrid,-1);
        imarisApplication.SetSurpassSelection(imarisApplication.GetSurpassScene.GetChild(imarisApplication.GetSurpassScene.GetNumberOfChildren-1));
        imarisApplication.GetSurpassSelection.SetColorRGBA(convertRGB2RGBA(0,1,1,0));
        %imarisApplication.GetSurpassSelection.SetVisible(0);
        ...
        %             anaText = [' - A: TP ' num2str(aStartFrame)];
        %         end
        imarisApplication.GetSurpassSelection.SetName('Pole Axis');
    end
...
%%  SUBFUNTIONS
    ...
    function trackCoords = getTrackCoords(track , noTimePoints)
        ...
        startTime_ = track.seqOfEvents(1,1); % get track start and end times
        endTime_ = track.seqOfEvents(2,1);
        ...
        trackCoords(startTime_:endTime_,:) = [X;Y;Z;dX;dY;dZ]';
    end
