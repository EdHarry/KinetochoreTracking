...
for iFrame = size(tracksIndxMatrix,2)+1 : numFrames
    tracksIndxMatrix = [tracksIndxMatrix zeros(size(tracksIndxMatrix,1),1)];%#ok<AGROW>
end
%% output
...
% updateClass depends on dataProperites, initCoords, planeFit, tracks and
% sisterList
% dependencies = struct('dataProperties',[],'initCoord',[],'planeFit',[],'tracks',[],'sisterList',[]);
...
% only add dependencies on tracks if it is present
if ~isempty(dataStruct.tracks)
    dependencies.tracks = tracksV;
end
% only add dependencies on sisterList if it is present
if ~isempty(dataStruct.sisterList)
    dependencies.sisterList = sisterListV;
end


% save into the first updatedClass
