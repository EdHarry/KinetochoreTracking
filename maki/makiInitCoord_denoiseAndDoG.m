function dataStruct = makiInitCoord_denoiseAndDoG(dataStruct, verbose, movieType, cutoffFix)
...
%fillZeroHoles = @(block_struct) fZH(block_struct.data);
...
for t=goodTimes
    ...
    if betterBackground
        ...
        %    rawMsk = imfill(rawMsk,'holes');
        %    for z=1:dataProperties.movieSize(3)
        %        rawMsk(:,:,z)=blockproc(rawMsk(:,:,z),[21 21],@fillZeroHoles);
        %        rawMsk(:,:,z)=blockproc(rawMsk(:,:,z),[21 21],fillZeroHoles);
        % imfill(BW,'holes')
        %    end
        rawMsk(rawMsk==0) = NaN;
        %   rawMsk = convNan(rawMsk,backgroundFilter,backgroundFilterParms(4:6),1);
        ...
        denoised = haarDenoise(raw,0.00008); % haar denoise
        
        
        % DoG fillter
        gamma=0.9;
        radius = 2;
        sigma    = ( radius / 2 / gamma ) * sqrt( ( 1 - gamma ^ 2 ) / ( -log( gamma ) ) );
        
        n=1.515;
        a = asin(dataProperties.NA/n);
        
        Dz = dataProperties.WVL/(2*n*(1-cos(a)));
        Dxy = dataProperties.WVL/(4*n*sin(a));
        
        alpha = (Dz*dataProperties.PIXELSIZE_XY)/(Dxy*dataProperties.PIXELSIZE_Z);
        sigmaZ = alpha * sigma;
        
        H = DoGMask3D( size( denoised ),[ sigma sigmaZ ], gamma );
        
        % Apply 3D DoG mask
        %   Perform convolution as multiplication in frequency domain,
        %   otherwise it gets too time consuming
        FH      = fftn( H );
        RAW  = fftn( double( denoised ) );
        FRAW = RAW .* FH;
        DoG_filtered = ifftshift( ifftn( FRAW ) );
        
        % Half-wave rectification
        DoG_filtered( DoG_filtered < 0 ) = 0;
...
%     error('bam')
    ...
    % find local maxima
    locMax = loc_max3Df(DoG_filtered,[3 3 3]);
...
for t=goodTimes
    ...
    % sort data by cutoffCol to really get two spots above and below 
    temp = initCoordRaw{t};
    temp = sortrows(temp,-cutoffCol);
    ...
    twoAbove = find(temp(:,cutoffCol) > cutoff(cutoffIdx),2,'last');
    twoBelow = find(temp(:,cutoffCol) < cutoff(cutoffIdx),2,'first');
    dataStruct.initCoord(t).data4MMF = ...
        temp([twoAbove;twoBelow],1:3);
end
...
% cordStruct = makiCoord2Cord(dataStruct.initCoord,1);
%[dataStruct.dataProperties] = ...
%     detectSpots_MMF_findAmplitudeCutoff(...
%rawMovie, cordStruct, dataStruct.dataProperties,[],0);

% cordStruct = makiCoord2Cord(dataStruct.initCoord);
%[~,testRatios] = ...
%     detectSpots_MMF_findAmplitudeCutoff(...
%rawMovie, cordStruct, dataStruct.dataProperties,[],0);

%dataStruct.dataProperties.testRatios = testRatios;
