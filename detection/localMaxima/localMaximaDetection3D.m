function [localMaxima,psfSigma] = ...
    localMaximaDetection3D(movieParam,detectionParam)
% finds localMaxima in 3D, based on detectSpots3D.m, uses splitModes.m 
...
% going to load in whole movie and store in memory, EHarry Oct 2012
movie = readOMEMatFile(imageName,imageIndx,imageCh,imageDecon,imageCrop);

% imageIndx should now index movie
imageIndx = 1:size(movie,4);
...
for t = 1:numImagesRaw
    ...
    imageTmp = movie(:,:,:,rollingAvStartEnd(t,1) : rollingAvStartEnd(t,2));
...
for iImage = goodImages
    ...
    try %try to detect features in this frame
        ...
        if strcmp(calcMethod,'g')
            featuresInfo = detectSubResFeatures3D_mlSparse_reCluster(imageRaw,...
