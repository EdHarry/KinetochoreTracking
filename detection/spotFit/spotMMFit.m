function [fullCoordList,fullAmpList,fullBgList,covarianceMatrix,newCentroidList,stats] = spotMMFit(frame,iniCoord,psfSize,varargin)
    ...
    for iCluster = 1:nCluster
        ...
        if(para.fitNPlusOne && ~isempty(newCoord))
            ...
            while(fitNPlusOne)
                ...
                % fit N+1
                [nPlusOneCoord,newResi,newAmp,newBg,nPlusOneJacobianMatrix] = mixtureModelFitting(nPlusOneCoord,maskAmp,maskCoord,psfSize,[],size(frame));
                ...
                if isempty(para.F_test_prob)
                    ...
                    F_test_prob = 1 - para.F_test_prob;
                end
                %%%%%%% PSEUDO WORK %%%%%%%%%%
                if para.debug
                    if nDim == 2
                    ...
                    if ~(isempty(discardDistanceIdx))
                        ...
                        if fitNPlusOne
                            ...
                            switch nDim
                                ...
                                case 1
                                    gMask1D = zeros(size(frame));
                                    for id = 1:size(newCoord,1)
                                        gMask1D = gMask1D +  GaussMask1D(psfSize,size(frame),newCoord(id,:),0)';
                                    end
                                    gMask1D = gMask1D > exp( - (para.influenceRadius^2 / 2));
                                ...
                                case 2
                                    ...
                                    gMask2D = zeros(size(frame));
                                    for id = 1:size(newCoord,1)
                                        gMask2D = gMask2D + GaussMask2D(psfSize,size(frame),newCoord(id,:),0,1,1);
                                    end
                                    if size(para.influenceRadius,2) == 1
                                        para.influenceRadius = [para.influenceRadius,para.influenceRadius];
                                    end
                                ...
                                case 3
                                    ...
                                    gMask3D = zeros(size(frame));
                                    for id = 1:size(newCoord,1)
                                        gMask3D = gMask3D + GaussMask3D(psfSize,size(frame),newCoord(id,:),0,1,1);
                                    end
                                    %Threshold of the influence radius of nSigma is n^2/2;
                                    %Exemple for 4*sigma : Gaussian =exp(-x^2/2*sigma^2) if x = 4*sigma -> exp(-8);
                                    if size(para.influenceRadius,2) == 1
                            ...
                            if size(newMaskCoord,1) ~= size(maskCoord,1) || ~all(newMaskCoord(:) == maskCoord(:))
                                %re-fit N+1 with new coords if they're different
