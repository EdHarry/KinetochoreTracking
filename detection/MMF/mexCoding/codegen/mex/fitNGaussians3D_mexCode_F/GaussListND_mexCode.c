...
void GaussListND_mexCode(const emxArray_real_T *coordList, real_T sigma, real_T center, emxArray_real_T *gaussList)
{
    ...
    c_emxInit_real_T(&b_center, 1, TRUE);
    iyLead = b_center->size[0];
    b_center->size[0] = iv2[0];
    emxEnsureCapacity((emxArray__common *)b_center, iyLead, (int32_T)sizeof(real_T));
    vstride = iv2[0] - 1;
    for (iyLead = 0; iyLead <= vstride; iyLead++) {
        b_center->data[iyLead] = center;
    }
    c_emxInit_real_T(&coordList2, 1, TRUE);
    iyLead = coordList2->size[0];
    coordList2->size[0] = vlen;
    emxEnsureCapacity((emxArray__common *)coordList2, iyLead, (int32_T)sizeof(real_T));
    vstride = vlen - 1;
    for (iyLead = 0; iyLead <= vstride; iyLead++) {
        coordList2->data[iyLead] = (coordList->data[iyLead] - b_center->data[iyLead]) / sigma;
    }
    emxFree_real_T(&b_center);
    c_emxInit_real_T(&varargin_1, 1, TRUE);
    ...
    for (dimSize = 0; dimSize < 2; dimSize++) {
        ysize[dimSize] = (int32_T)sz1[dimSize];
    }
