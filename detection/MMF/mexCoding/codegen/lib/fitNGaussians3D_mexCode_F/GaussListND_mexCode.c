 ...
 *
 * C source code generated on: Mon May 28 14:50:34 2012
 *
 */
  ...
  center, emxArray_real_T *gaussList)
{
  ...
  /* 'GaussListND_mexCode:42' if isempty(coordList) */
  if (coordList->size[0] == 0) {
    /* 'GaussListND_mexCode:43' error('you have to supply a list of coordinates for GaussList23D') */
  } else {
    /* 'GaussListND_mexCode:44' else */
    /* 'GaussListND_mexCode:45' [nCoords,nDims] = size(coordList); */
  ...
  /*      end */
  /*  end */
  /* 'GaussListND_mexCode:75' center2 = repmat(center, [nCoords,1]); */
  b_nCoords[0] = nCoords;
  b_nCoords[1] = 1;
  ...
  /* clear center */
  /*  intNorm */
  /* 'GaussListND_mexCode:79' if nIn < 4 || isempty(intNorm) */
  /* 'GaussListND_mexCode:80' intNorm = 0; */
  /* rotation */
  ...
  /* 'GaussListND_mexCode:161' center3 = center2(:,1); */
  /* clear center2 */
  /*  convert coordList to 0/1 */
  /* 'GaussListND_mexCode:165' coordList2 = (coordList(1:nCoords) - center3(1:nCoords))./sigma; */
  ...
  coordList2->size[0] = nCoords;
  emxEnsureCapacity((emxArray__common *)coordList2, vstride, (int32_T)(sizeof
    (real_T)));
  stride = nCoords - 1;
  for (vstride = 0; vstride <= stride; vstride++) {
    coordList2->data[vstride] = (coordList->data[vstride] - b_center->
      data[vstride]) / sigma;
  }
  ...
  /* 'GaussListND_mexCode:170' coordList2 = cat(3,coordList2-0.5./sigma, coordList2+0.5./sigma); */
  z = 0.5 / sigma;
  p = 0.5 / sigma;
  ...
  ysize[2] = 2U;
  vstride = (b_coordList2->size[0] * b_coordList2->size[1]) * b_coordList2->
    size[2];
  b_coordList2->size[0] = (int32_T)ysize[0];
  b_coordList2->size[1] = (int32_T)ysize[1];
  ...
  /* 'GaussListND_mexCode:174' gaussList = diff(0.5 * erfc(-coordList2/sqrt(2)),1,3); */
  vstride = (b_coordList2->size[0] * b_coordList2->size[1]) * b_coordList2->
    size[2];
    ...
    if (rtIsNaN(b_coordList2->data[k])) {
      ...
      if (absx < 1.3877787807814457E-17) {
        ...
        p = (0.12837916709551256 + (z * (-0.3250421072470015 + (z *
                (-0.02848174957559851 + (z * (-0.0057702702964894416 + (z *
                    -2.3763016656650163E-5)))))))) / (1.0 + (z *
          (0.39791722395915535 + (z * (0.0650222499887673 + (z *
          (0.0050813062818757656 + (z * (0.00013249473800432164 + (z *
      ...
      if (b_coordList2->data[k] >= 0.0) {
        p = 0.15493708848953247 - ((-0.0023621185607526594 + ((absx - 1.0) *
          ...
          (0.10642088040084423 + ((absx - 1.0) * (0.540397917702171 + ((absx -
          1.0) * (0.071828654414196266 + ((absx - 1.0) * (0.12617121980876164 +
          ((absx - 1.0) * (0.013637083912029051 + ((absx - 1.0) *
        ...
        p = 1.0 + (0.84506291151046753 + ((-0.0023621185607526594 + ((absx - 1.0)
          * (0.41485611868374833 + ((absx - 1.0) * (-0.37220787603570132 +
          ...
          (0.10642088040084423 + ((absx - 1.0) * (0.540397917702171 + ((absx -
          1.0) * (0.071828654414196266 + ((absx - 1.0) * (0.12617121980876164 +
                             ((absx - 1.0) * (0.013637083912029051 + ((absx -
      ...
      if (absx < 2.8571414947509766) {
        ...
        S = 1.0 + (p * (30.338060743482458 + (p * (325.79251299657392 + (p *
          (1536.729586084437 + (p * (3199.8582195085955 + (p *
          (2553.0504064331644 + (p * (474.52854120695537 + (p *
          -22.440952446585818)))))))))))));
      }
      ...
      z = (floor(p * 2.097152E+6) / 2.097152E+6) * rt_powd_snf(2.0, (real_T)
        nCoords);
      p = (exp(((-z) * z) - 0.5625) * exp(((z - absx) * (z + absx)) + (R / S))) /
        absx;
      if (b_coordList2->data[k] < 0.0) {
        p = 2.0 - p;
      }
  ...
  for (vstride = 0; vstride < 3; vstride++) {
    ysize[vstride] = (uint32_T)y->size[vstride];
  }
  ...
  /* 'GaussListND_mexCode:175' gaussList = prod(gaussList,2); */
  for (vstride = 0; vstride < 3; vstride++) {
    ysize[vstride] = (uint32_T)b_y1->size[vstride];
  }

  ysize[1] = 1U;
  vstride = (gaussList->size[0] * gaussList->size[1]) * gaussList->size[2];
  gaussList->size[0] = (int32_T)ysize[0];
  gaussList->size[1] = 1;
  gaussList->size[2] = 1;
  ...
  /*  norm gaussList */
  /* 'GaussListND_mexCode:178' switch intNorm */
  /* 'GaussListND_mexCode:179' case 0 */
  /*  "un-norm" Gaussian */
  /* 'GaussListND_mexCode:181' gaussList = gaussList*((2*pi)^(0.5*nDims)*prod(sigma(1,:))); */
