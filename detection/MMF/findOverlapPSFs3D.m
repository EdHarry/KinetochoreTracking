function [clusters,errFlag] = findOverlapPSFs3D(cands,numPixelsX,numPixelsY,numPixelsZ,...
...
tmp = sub2ind([numPixelsY,numPixelsX,numPixelsZ],cands2(:,1),cands2(:,2),cands2(:,3));
maxPos0 = zeros(numPixelsY,numPixelsX,numPixelsZ);
maxPos0(tmp) = 1;
...
image = zeros(numPixelsY+2*psfRange(1),numPixelsX+2*psfRange(1),numPixelsZ+2*psfRange(2));
% imageAmp = zeros(numPixelsY+2*psfRange,numPixelsX+2*psfRange);
imageAmp = zeros(numPixelsY+2*psfRange(1),numPixelsX+2*psfRange(1),numPixelsZ+2*psfRange(2));
for i=1:size(cands2,1)
    ...
    image(ymin:ymax,xmin:xmax,zmin:zmax) = image(ymin:ymax,xmin:xmax,zmin:zmax) + template;
    imageAmp(ymin:ymax,xmin:xmax,zmin:zmax) = imageAmp(ymin:ymax,xmin:xmax,zmin:zmax) + template*candsAmp(i);
end
...
for i=1:nIsland
    ...
    [rcCenter1,rcCenter2,rcCenter3] = ind2sub([numPixelsY numPixelsX numPixelsZ],rcCenterL);
    %rcCenter = [ceil(rcCenterL/numPixelsY) mod(rcCenterL,numPixelsY)];
    [rcInd1,rcInd2,rcInd3] = ind2sub([numPixelsY numPixelsX numPixelsZ],rc);
