function [detectedFeatures,clustersMMF,imageN3,errFlag] = ...
    detectSubResFeatures3D_mlSparse(image,cands,psfSigma,testAlpha,visual,...
...
[clusters,errFlag] = findOverlapPSFs3D_new(cands,numPixelsX,numPixelsY,numPixelsZ,psfSigma);
if errFlag
...
for i = 1 : numClusters
    ...
    if numMaximaT > 1
        ...
        %
        %         %find the minimum distance for each maximum
        %         [distBetweenMaxSort,distBetweenMaxSortIdx] = sort(distBetweenMax,2);
        %         distBetweenMaxSortIdx = distBetweenMaxSortIdx(:,2:end);
        %         distBetweenMaxSort = distBetweenMaxSort(:,2:end);
        %         %minDistBetweenMaxIdx = distBetweenMaxSortIdx(:,1);
        %         minDistBetweenMax = distBetweenMaxSort(:,1);
        %
        %         % XYdistances
        %         distBetweenMaxXY = createDistanceMatrix(maximaPosT(:,1:2),maximaPosT(:,1:2));
        %         distBetweenMaxXY_temp = distBetweenMaxXY(:,2:end);
        %         for iRow = 1:size(distBetweenMaxXY,1)
        %             distBetweenMaxXY_temp(iRow,:) = distBetweenMaxXY(iRow,distBetweenMaxSortIdx(iRow,:));
        %         end
        %         distBetweenMaxXYSort = distBetweenMaxXY_temp;
        %         minDistBetweenMaxXY = distBetweenMaxXYSort(:,1);
        %
        %         % Zdistances
        %         distBetweenMaxZ = abs(createDistanceMatrix(maximaPosT(:,3),maximaPosT(:,3)));
        %         distBetweenMaxZ_temp = distBetweenMaxZ(:,2:end);
        %         for iRow = 1:size(distBetweenMaxZ,1)
        %             distBetweenMaxZ_temp(iRow,:) = distBetweenMaxZ(iRow,distBetweenMaxSortIdx(iRow,:));
        %         end
        %         distBetweenMaxZSort = distBetweenMaxZ_temp;
        %         minDistBetweenMaxZ = distBetweenMaxZSort(:,1);
        %
        %         %         %find the minimum minimum distance
        %         %         minMinDistBetweenMax = min(minDistBetweenMax);
        %
        %         %find the minimum minimum distance
        %         [minMinDistBetweenMax,minMinDistBetweenMaxIdx] = min(minDistBetweenMax);
        %         minMinDistBetweenMaxXY = minDistBetweenMaxXY(minMinDistBetweenMaxIdx);
        %         minMinDistBetweenMaxZ = minDistBetweenMaxZ(minMinDistBetweenMaxIdx);
        %
        ...
        %
        %         %if this distance is smaller than 2*psfSigma in xy or psfSigma in z, remove the maximum with
        %         %smallest average distance to its neighbors
        %         while minMinDistBetweenMaxXY <= (2 * psfSigma(1)) && minMinDistBetweenMaxZ <= psfSigma(2)
        %
        ...
        %                 minMinDistBetweenMaxZ = minDistBetweenMaxZ(minMinDistBetweenMaxIdx);
        %             else
        %                 minMinDistBetweenMaxXY = 3 * psfSigma(1);
        %                 minMinDistBetweenMaxZ = 3 * psfSigma(2);
        %             end
        %
        %         end
    ...
    while fit
        ...
        %    clusterPixels,psfSigma);
        [solutionT,residualsT,jacMatT] = fitNGaussians3D_mexCode_fitFun(options,x0, lb, ub, imageC, clusterPixels, psfSigma, [numPixelsX numPixelsY numPixelsZ], clusters(i).pixels(:,4));
    ...
    if keepCluster(i)
        ...
        while testAmp
            ...
            if numMaxima > 1
                ...
                [solution,residuals,jacMat] = fitNGaussians3D_mexCode_fitFun(options, x0, lb, ub, imageC, clusterPixels, psfSigma, [numPixelsX,numPixelsY,numPixelsZ], clusters(i).pixels(:,4));
                jacMat = full(jacMat);
...
if ~isempty(indx)
    ...
    for iCluster = 1 : length(clustersMMF)
        ...
        pValue = 1 - fcdf(testStat,numDegFree,numDegFree+4*size(clustersMMF(iCluster).bgAmp,1));
    ...
    if ~isempty(indx)
        ...
        %store information in structure "detectedFeatures"
        tmp = vertcat(clustersMMF.position);
        detectedFeatures.xCoord = [tmp(:,1) tmp(:,4)];
        detectedFeatures.yCoord = [tmp(:,2) tmp(:,5)];
        detectedFeatures.zCoord = [tmp(:,3) tmp(:,6)];
        detectedFeatures.amp = vertcat(clustersMMF.amplitude);
...
if isempty(indx)
    ...
    clustersMMF = struct('position',zeros(0,6),'amplitude',zeros(0,2),'bgAmp',zeros(0,2));
