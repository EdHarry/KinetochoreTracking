function dataStruct = makiAlignAllFrames_V2(dataStruct)
...
for iFrame = 1 : numFrames
    ...
    coords = allCoords(iFrame).coords(:,1:3) - repmat(centerOfMass_o(iFrame,:),size(allCoords(iFrame).coords(:,1:3),1),1);
...
%% function to get plane coords of two tracks
    function [coord1, coord2] = planeCoords(allCoords,com,coordSystem,iFrame,jFrame,tracksIndx)
        ...
        % make rotated coords
        coord1 = (coordSystem(:,:,iFrame) \ coord1')';
        coord2 = (coordSystem(:,:,jFrame) \ coord2')';
