function dataStruct = makiGenerateTracks_frameAlignment(dataStruct)
...
if dataStruct.dataProperties.tracksParam.rotate == 1
    if ~isempty(dataStruct.frameAlignment)
        ...
        for iTime = 1 : nTimepoints
            allCoord = dataStruct.frameAlignment(iTime).alignedCoord;
            if ~isempty(allCoord)
    ...
    elseif ~isempty(dataStruct.planeFit)
        %get the rotated coordinates
        for iTime = 1 : nTimepoints
    ...
    else % original coords com shifted
        ...
        for iTime = 1 : nTimepoints
            if ~isempty(dataStruct.initCoord(iTime).allCoord)
...
if ~isempty(dataStruct.updatedClass) %if the classes have been updated
    ...
    for iTime = 1 : nTimepoints
        ...
        kinType(updatedClass(iTime).unalignedIdx) = 1; %unaligned
        kinType(updatedClass(iTime).laggingIdx) = 2; %lagging
        movieInfo(iTime).kinType = kinType;
    end
    
elseif ~isempty(dataStruct.planeFit) %if the plane fit has been done
...
%   tracks = trackCloseGapsKalman(movieInfo,... % verbose tracking
%       costMatrices,gapCloseParam,kalmanFunctions,3,0,1);
...
% tracks depends on dataProperites, initCoords and planeFit
% dependencies = struct('dataProperties',[],'initCoord',[],'planeFit',[]);
