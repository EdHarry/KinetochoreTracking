function dataStruct = KTPairTracking_copy(dataStruct)
...
while fittingLoop2
    ...
    while fittingLoop1
        ...
        for iFrame = 1:nFrames
            ...
            % if no new spots then continue
            if size(coordList,1) == initCoord(iFrame).nSpots;
                progressText(iFrame/nFrames,['re-fitting for new spots: no new spots for frame ' int2str(iFrame)])
                continue
            end
            % else match up previous coords to new coord, then add new coords to
            % end of initCoord, then calculate new aligned coords
            newSpotAdded = true;
