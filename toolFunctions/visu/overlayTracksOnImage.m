function overlayTracksOnImage( dataStruct, tracks2Plot )
...
if nargin < 2 || isempty(tracks2Plot)
    tracks2Plot = 1:length(dataStruct.tracks);
end

global tracks2PlotTmp
tracks2PlotTmp = tracks2Plot;

% load tracks and coords
tracks = dataStruct.tracks;
if ~isempty(tracks2Plot)
    tracks = tracks(tracks2Plot);
end
initCoord = dataStruct.initCoord;


% get image coordinate of tracks
coords(1:length(tracks)) = struct('imageCoords',[]);
for i = 1:length(tracks)
    imageCoords = NaN(frames,3);
    featIdx = getFeatIdx(tracks(i),frames);
    for t = 1:frames
        if ~isnan(featIdx(t))
            imageCoords(t,:) = initCoord(t).allCoordPix(featIdx(t),1:3);
        end
    end
    coords(i).imageCoords = imageCoords;
end
    ...
    function plotPlanes(figH,initialPlot,key)
        ...
        if ~isempty(spots)
            ...
            for iTe = 1:size(spots,1)
                ...
                col = [0 1 0];
                text(textCoord(2),textCoord(1),['\color[rgb]{',num2str(col(1)),' ',num2str(col(2)),' ',num2str(col(3)),'}','Track ID ',int2str(tracks2PlotTmp(tracks4Plane(iTe)))])
            end
            hold off
        end
