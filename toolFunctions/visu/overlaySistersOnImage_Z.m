function overlaySistersOnImage_Z( dataStruct, sis2Plot )
...
nZSec = dataStruct.dataProperties.movieSize(1);
...
%% LOCAL FUNCTIONS
    ...
    function plotPlanes(figH,initialPlot,key)
        ...
        %%%%%
        image(58:64,:,:) = repmat(max(image(58:64,:,:)),7,1,1);
        %%%%%
        
        maxIn = max(image(:));
        minIn = min(image(:));
        image = squeeze(image(currentZSec,:,:))';
        %image = image(:,end:-1:1);
        ...
        idx = spots(:,1) < (currentZSec+6) & spots(:,1) > (currentZSec-6);
        tracks4Plane = find(idx);
        %         spots = spots((spots(:,3) < (currentZSec+0.5) & spots(:,3) > (currentZSec-0.5)),:);
        spots = spots(idx,:);
        
        for iTr = 1:size(trackSeg,2)
            if ~(trackSeg(toPlot+1,iTr,1) < (currentZSec+6) && trackSeg(toPlot+1,iTr,1) > (currentZSec-6));
                trackSeg(:,iTr,:) = NaN;
            end
        end
        ...
        if ~isempty(spots)
            hold on
            plot(spots(:,2),spots(:,3),'g*');
            plot(trackSeg(1:toPlot+1,:,2),trackSeg(1:toPlot+1,:,3),'r')
            plot(trackSeg(toPlot+1:end,:,2),trackSeg(toPlot+1:end,:,3),'b')
            
            for iTe = 1:size(spots,1)
                textCoord = spots(iTe,2:3) + [-1 1];
                col = [0 1 0];
                ...
                text(textCoord(1),textCoord(2),['\color[rgb]{',num2str(col(1)),' ',num2str(col(2)),' ',num2str(col(3)),'}','Sister Pair ID ',int2str(sis2PlotTmp(sisPairNo)),':',int2str(sisNo)])
            end
            hold off
        end
